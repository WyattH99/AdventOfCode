...
fn get_sides(region: &Vec<Vec2>) -> usize {
    // Vec of (pos and dir)
    // This is basically the outline of the region
    let mut sides: Vec<(Vec2, Vec2)> = region
        .iter()
        .flat_map(|&pos| {
            dirs()
                .into_iter()
                .filter(move |dir| !region.contains(&(pos + dir)))
                .map(move |dir| (pos, dir))
        })
        .collect();

    // combine all the sides that are connected and have the same dir
    let mut sides_num = 0;
    while let Some(part) = sides.pop() {
        reduce_sides(part, &mut sides);
        sides_num += 1;
    }
...
fn reduce_sides(part: (Vec2, Vec2), sides: &mut Vec<(Vec2, Vec2)>) {
    dirs().into_iter().for_each(|dir| {
        let pos = part.0 + dir;
        if let Some(index) = sides
            .iter()
            .position(|part2| part2.1 == part.1 && part2.0 == pos)
        {
            let part2 = sides.remove(index);
            reduce_sides(part2, sides);
        }
    });
}
