...
impl Grid {
    ...
    fn can_push(&self, pos: Vec2, dir: Vec2) -> bool {
        ...
        match next_c {
            ...
            _ => {
                if dir.x != 0 {
                    // moving vertically
                    let b0 = self.can_push(next, dir);
                    let b1 = if next_c == '[' {
                        self.can_push(next + Vec2::new(0, 1), dir)
                    } else {
                        self.can_push(next + Vec2::new(0, -1), dir)
                    };
                    b0 && b1
                    ...
                    // moving horizontally
                    let next_next = next + dir;
                    self.can_push(next_next, dir)
                }
            }
        }
    }
    ...
    fn push(&mut self, pos: Vec2, dir: Vec2) {
        ...
        if next_c != '.' {
            // keep looking for empty space
            //self.push(next, dir);
            if dir.x != 0 {
                // moving vertically
                self.push(next, dir);
                if next_c == '[' {
                    self.push(next + Vec2::new(0, 1), dir);
                } else {
                    self.push(next + Vec2::new(0, -1), dir);
                };
                ...
                // moving horizontally
                let next_next = next + dir;
                self.push(next_next, dir);
                self.set(next_next, self.get(next).unwrap());
                self.set(next, current_c);
                self.set(pos, '.');
            }
        } else {
