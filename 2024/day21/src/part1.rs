...
fn main() {
    ...
    */
    ...
    let result: usize = codes
        .iter()
        .map(|code| {
            let len = get_numpad_code_length(code, 2);
...
fn get_numpad_code_length(code: &str, depth: usize) -> usize {
    ...
    chars
        ...
        .map(|(&from, &to)| {
            ...
            paths
                .iter()
                .map(|path| get_keypad_code_length(path, depth))
                .min()
                .unwrap()
        })
        .sum()
...
fn get_keypad_code_length(path: &str, depth: usize) -> usize {
        ...
        chars
            ...
            .map(|(&from, &to)| {
                if from != to {
                    ...
                    paths
                        .iter()
                        .map(|path| get_keypad_code_length(path, depth - 1))
                        .min()
                        .unwrap()
                } else {
                    1
