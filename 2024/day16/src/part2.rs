...
fn solve(grid: &Grid) -> usize {
    ...
    let mut open = vec![(pos, dir, 0, None)];
    let mut ends = Vec::new();
    let mut to_from: HashMap<(Vec2, Vec2), Vec<(Vec2, Vec2)>> = HashMap::new();
    ...
    while open.len() > 0 {
            ...
            if prev_visit_cost <= cost {
                if prev_visit_cost == cost {
                    if let Some(from) = from {
                        to_from.get_mut(&key).unwrap().push(from);
                    }
                }
                continue;
            }
        ...
        if let Some(from) = from {
            to_from.insert(key, vec![from]);
        } else {
            to_from.insert(key, vec![]);
        }
        ...
        open.push((pos + dir, dir, cost + 1, Some(key)));
        // Can only turn 90 degrees each time
        let turn_dirs = turns(dir);
        open.push((pos + turn_dirs.0, turn_dirs.0, cost + 1001, Some(key)));
        open.push((pos + turn_dirs.1, turn_dirs.1, cost + 1001, Some(key)));
    ...
    let lowest = ends.iter().map(|e| e.1).min().unwrap();
    for (e, _) in ends.iter().filter(|e| e.1 == lowest) {
        stack.push(e);
    }

    while let Some(key) = stack.pop() {
        seats.insert(key.0);
        if let Some(v) = to_from.get(key) {
            stack.extend(v);
        }
    }
    seats.len()
